...
def calculate_inverse_kinematics(pos, rotation, rel_pos=np.array([0.0, 0, 0.25]), rel_mat=np.identity(3)):
    pos, rotation = angle_math.cood_b_to_a(pos, rotation, rel_pos, rel_mat)
    transform_matrix = to_transformation_matrix(pos_trans(pos), mat_trans(rotation))
    num_sols, q_sols_raw = inverse(transform_matrix, 0)
...
if __name__ == '__main__':
    ...
    rot_matrix = np.array([[0, 0, 1],
                           [1, 0, 0],
                           [0, 1, 0]])
    # num_sols, q_sols = calculate_inverse_kinematics(np.array([-0.7, 0, 0.3]), np.identity(3))
    num_sols, q_sols = calculate_inverse_kinematics(np.array([-0.7, 0, 0.3]), rot_matrix)
    print(num_sols)
    print(q_sols)
    print(q_sols * 180 / np.pi)
